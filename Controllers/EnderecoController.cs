    ...
    public class EnderecoController : Controller {
        ...
        public async Task<IActionResult> Cadastrar(int? cid, int? eid)
        {
            if(cid.HasValue)
            {
                ...
                if(cliente !=  null)
                {
                    ...
                    if(eid.HasValue)
                    {
                        _context.Entry(cliente).Collection(o => o.Enderecos).Load();
            ...
            else
            {
                ...
                TempData["mensagem"] = MensagemModel.Serializar("Nenhum proprietário de enderecos foi informado", TipoMensagem.Erro);
                return RedirectToAction("Index", "Cliente");
            }
        }
        ...
        public async Task<IActionResult> Excluir(int idUsuario, int idEndereco)
        {
            ...
            if(endereco != null)
            {
                ...
                if(await _context.SaveChangesAsync() > 0)
                {
                    TempData["mensagem"] = MensagemModel.Serializar("Endereço removido com sucesso !");
                ...
                {
                    TempData["mensagem"] = MensagemModel.Serializar("Erro ao remover endereço", TipoMensagem.Erro);
                }
            }
            else
            {
